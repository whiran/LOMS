...
import { getcusordersprocess, getorders, getprocessingorders } from '@/app/actions/api/getorders';
...
const page = async (props: Props) => {
  ...
  const fetcheddata = await getcusordersprocess(userid);
  return (
    ...
    <div className="h-[9vh]">
     ...
     <div>
     <h1 className='font-bold text-center'>Processing Order Details:</h1>
       ...
       <tbody>
         {fetcheddata.map(data => (
           <tr key={data.id} className="border-b-2 hover:bg-slate-300">
             ...
             <td className='w-6'><Link href={`/protected/misreport/process/${data.id}`}><Text className='hover:text-cyan-400'/></Link></td>
           </tr>
         ))}
       </tbody>
     </table>
