...
const Orderqty = (props: Props) => {
  ...
  const handleSubmit = async(orderId: string) => {
    ...
    const resut = await orderqty(orderId,quantity);
    setBoolval(!boolval);
  };
  return (
        ...
        <table className='overflow-auto w-full'>
          ...
          <tbody>
            ...
            <tr key={order.id} className='border-t-1'>
              ...
              <td>{order.state}</td>
              <td className='w-24'>{(order.ordervalue  !== null)? (<input type='number' required placeholder='Quantity'  onChange={(e) => handleQuantityChange(order.id, parseInt(e.target.value))} value={order.ordervalue}/>):(<input type='number' required placeholder='Quantity'  onChange={(e) => handleQuantityChange(order.id, parseInt(e.target.value))}/>)}</td>
